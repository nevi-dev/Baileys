...
async function getAudioWaveform(buffer, logger) {
    try {
        ...
        const samples = 64 // Tetap 64
        const blockSize = Math.floor(rawData.length / samples)
        const filteredData = []

        // --- © RexxHayanasi ---
        //cari nilai PUNCAK (peak), bukan RATA-RATA (average)
        for (let i = 0; i < samples; i++) {
            let peak = 0.0 // Mulai dari 0
            const blockStart = blockSize * i
            for (let j = 0; j < blockSize; j++) {
                const amp = Math.abs(rawData[blockStart + j] || 0)
                if (amp > peak) {
                    peak = amp // Simpan nilai tertinggi di segmen ini
                }
            }
            filteredData.push(peak) // Masukkan nilai puncak
        }
        // --- BATAS PERUBAHAN ---

        // normalisasi ke rentang 0–255
        const max = Math.max(...filteredData) || 1
        const normalizedData = filteredData.map(n => (n / max) * 255)

        // pastikan hasilnya Uint8Array panjang tetap
        const waveform = new Uint8Array(samples)
        for (let i = 0; i < samples; i++) {
            waveform[i] = Math.max(0, Math.min(255, Math.round(normalizedData[i])))
        }

        return waveform
    } catch (e) {
        logger?.debug?.('⚠️ Failed to generate waveform: ' + e)
        // fallback agar gak undefined
        return new Uint8Array(64).fill(0) // fallback 64 titik
    }
}
